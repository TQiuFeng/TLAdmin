 ...
 * 演示接口直接返回数组(框架自动包装统一响应);正式业务建议返回 VO,以生成完整的 OpenAPI 结构。
 * Author: qiufeng
 */
...
final class DemoToolboxController extends BaseController
{
    ...
    public function samples(): array
    {
        ...
        return [
            ...
            'region' => [
                ...
                'levels' => array_map(fn (array $rows): array => $this->withLeaf($rows), $levels),
    ...
    public function money(): array
    {
        ...
        return $this->buildMoney($amount);
    }

    #[GetMapping('/regions', permission: 'demo-toolbox:use', summary: '行政区划:下级列表(不传 code 返回省份)', query: [
    ...
    public function regions(): array
    {
        ...
        return $this->withLeaf($code === '' ? Tools::regionProvinces() : Tools::regionChildren($code));
    }
    ...
    public function regionDetail(): array
    {
        return $this->buildRegionDetail((string) $this->query('code', ''));
    }
    ...
    public function qrcode(): array
    {
        ...
        return $this->buildQrcode($content);
    }
