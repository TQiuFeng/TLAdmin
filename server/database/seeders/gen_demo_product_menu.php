...
return [
    'run' => static function (): bool {
        if (Db::table('tl_admin_menu')->where('permission', 'demo-product:list')->find()) {
            return false;
        }
        ...
        $menuId = Db::table('tl_admin_menu')->insertGetId([
            'parent_id' => 0, 'type' => 'menu', 'title' => '演示商品',
            'name' => 'DemoProduct', 'path' => '/gen/demo-product', 'component' => 'gen/demo-product/index',
            'icon' => 'view-module', 'permission' => 'demo-product:list', 'sort' => 90,
        ...
        foreach ([['新增', 'create'], ['编辑', 'update'], ['删除', 'delete']] as $i => [$title, $action]) {
            Db::table('tl_admin_menu')->insert([
                ...
                'permission' => "demo-product:{$action}", 'sort' => $i,
                'visible' => 1, 'status' => 1, 'create_time' => $now, 'update_time' => $now,
            ]);
        }
