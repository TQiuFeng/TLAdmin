 ...
 * 操作日志迁移到 MongoDB。
 *
 * 为 operation_log 集合建索引(username、create_time),
 * 并把 tl_operation_log 表的存量数据搬到 Mongo 后删表。
 * Author: qiufeng
 */
...
return [
    'up' => static function (): void {
        ...
        $lastId = 0;
        while (true) {
            $rows = Db::table('tl_operation_log')
                ->where('id', '>', $lastId)
                ->order('id')
                ->limit(500)
                ->select()
        ...
        Db::execute('DROP TABLE IF EXISTS `tl_operation_log`');
    },
];
